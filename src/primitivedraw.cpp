...
void DrawPoint(const normalizedPoint& p, float objectDiffuse[4])
{
...
void DrawLine(double x1, double y1, double z1, double x2, double y2, double z2)
{
	...
	DrawLine(x1, y1, z1, x2, y2, z2, objectDiffuse);
}

void DrawLine(double x1, double y1, double z1, double x2, double y2, double z2, float objectDiffuse[4])
{
	glMaterialfv( GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, objectDiffuse );
	glLineWidth(5.f);
...
void DrawArrow(const normalizedPoint& p, const vectorE3GA& dir, float objectDiffuse[4])
{
	double length = 1;
	...
	::DrawLine(p.e1(), p.e2(), p.e3(), p2.e1(), p2.e2(), p2.e3(), objectDiffuse);
}

void DrawArrow(const normalizedPoint& p, const vectorE3GA& dir)
{
	float		objectDiffuse[] = { 1.0f, 0.f, 0.f, 1.f };
	DrawArrow(p, dir, objectDiffuse);
}
