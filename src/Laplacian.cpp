...
std::shared_ptr<SparseMatrix<double>> CreateLaplacianMatrix(Mesh *mesh, int type)
{
	std::shared_ptr<SparseMatrix<double>> L(new SparseMatrix<double>(mesh->numVertices()));
	...
	{
		SparseMatrix<double>::mapVect::iterator iter = L->matrix[i].begin();
		SparseMatrix<double>::mapVect::iterator endIter = L->matrix[i].end();
